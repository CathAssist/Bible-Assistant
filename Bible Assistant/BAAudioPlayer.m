...
- (void)setIsPlaying:(BOOL)isPlaying
{
    ...
    if (_isPlaying) {
        [[NSNotificationCenter defaultCenter] postNotificationName:[NSString stringWithFormat:@"Volume%ldChapter%ldStateButtonTitle", (long)[self.volumeNum integerValue], (long)[self.chapterNum integerValue]] object:nil userInfo:@{@"buttonTitle":@"暂停"}];
    } else {
        [[NSNotificationCenter defaultCenter] postNotificationName:[NSString stringWithFormat:@"Volume%ldChapter%ldStateButtonTitle", (long)[self.volumeNum integerValue], (long)[self.chapterNum integerValue]] object:nil userInfo:@{@"buttonTitle":@"播放"}];
    }
}
...
- (void)updateLocalProgress:(NSTimer*)timer {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:[NSString stringWithFormat:@%ldolume%dChapter%ldAudioPlayProg(long)ress", [self.volumeNum integerValue], (long)[self.chapterNum integerValue]] object:nil userInfo:@{@"progress":@(progress)}];
...
- (void)playerItemDidReachEnd:(NSNotification *)notification {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:[NSString stringWithFormat:@%ldolume%dChapter%ldAudioPlayProg(long)ress", [self.volumeNum integerValue], (long)[self.chapterNum integerValue]] object:nil userInfo:@{@"progress":@(1.0)}];
